...
if activo_escogido:    
     ...
     VaR95_rolling_df = pd.DataFrame({'Date': df_rendimientos[activo_escogido[0]].index, '95% VaR Rolling': VaR95_rolling_percent.squeeze()})
     VaR95_rolling_df.set_index('Date', inplace=True)
     
     #Cálculos de VaR al 99% de confianza, creación de dataframe para poder graficarlo
     VaR99_rolling = norm.ppf(1-0.99, rolling_mean, rolling_std)
     VaR99_rolling_percent = (VaR99_rolling * 100).round(4)
     VaR99_rolling_df = pd.DataFrame({'Date': df_rendimientos[activo_escogido[0]].index, '99% VaR Rolling': VaR99_rolling_percent.squeeze()})
     VaR99_rolling_df.set_index('Date', inplace=True)
     
     #Cálculo de hVaR al 95% de confianza, creación de dataframe para graficarlo y poder sacar el ES
     hVaR95_rolling = (df_rendimientos[activo_escogido[0]].rolling(window = 252).quantile(1-0.95))
     hVaR95_rolling_percent = (hVaR95_rolling * 100).round(4)
     hVaR95_rolling_df = pd.DataFrame({'Date': df_rendimientos[activo_escogido[0]].index, '95% hVaR Rolling': hVaR95_rolling_percent.squeeze()})
     hVaR95_rolling_df.set_index('Date', inplace=True)
     
     #Cálculo de hVaR al 99% de confianza, creación de dataframe para graficarlo y poder sacar el ES
     hVaR99_rolling = (df_rendimientos[activo_escogido[0]].rolling(window = 252).quantile(1-0.99))
     hVaR99_rolling_percent = (hVaR99_rolling * 100).round(4)
     hVaR99_rolling_df = pd.DataFrame({'Date': df_rendimientos[activo_escogido[0]].index, '99% hVaR Rolling': hVaR99_rolling_percent.squeeze()})
     hVaR99_rolling_df.set_index('Date', inplace=True)

     

   
# Cálculo del VaR al 95% (rolling)
     z_95 = norm.ppf(0.95)  # Percentil 95% de la normal estándar
     VaR95_rolling = rolling_mean - z_95 * rolling_std  # VaR al 95%

# Cálculo del CVaR al 95% (Expected Shortfall)
     CVaR95_rolling = rolling_mean - (norm.pdf(z_95) / (1 - 0.95)) * rolling_std

# Convertir a porcentaje
     CVaR95_rolling_percent = (CVaR95_rolling * 100).round(4)

# Crear DataFrame para graficar
     CVaR95_rolling_df = pd.DataFrame({
         'Date': df_rendimientos.index,  # Asegurar que se usa el índice correcto
         '95% CVaR Rolling': CVaR95_rolling_percent.squeeze()})
...
# Establecer índice de fecha
     ...
     #CVaR95_rolling= rolling_mean + (norm.cdf(df_rendimientos[activo_escogido[0]].rolling(window= 252).quantile(1-0.95), rolling_mean, rolling_std)/(1-0.95))*rolling_std
     #CVaR95_rolling_percent = (CVaR95_rolling * 100).round(4)
     ...
     #Cálculo de CVaR al 99% de confianza, creación de dataframe para graficarlo
     CVaR99_rolling= (rolling_mean - (norm.cdf(df_rendimientos[activo_escogido[0]].rolling(window= 252).quantile(1-0.99), rolling_mean, rolling_std)/(1-0.99))*rolling_std)
     CVaR99_rolling_percent = (CVaR99_rolling * 100).round(4)
     CVaR99_rolling_df = pd.DataFrame({'Date': df_rendimientos[activo_escogido[0]].index, '99% CVaR Rolling': CVaR99_rolling_percent.squeeze()})
     CVaR99_rolling_df.set_index('Date', inplace=True)

     #Cálculo del hCVaR al 95% de confianza, creación de dataframe para graficarlo
     hCVaR95_rolling = df_rendimientos[activo_escogido[0]].rolling(window=252).apply(lambda x: x[x <= hVaR95_rolling.loc[x.index[-1]]].mean(), raw=False)
     hCVaR95_rolling_percent = (hCVaR95_rolling * 100).round(4)
     hCVaR95_rolling_df = pd.DataFrame({'Date': df_rendimientos[activo_escogido[0]].index, '95% hCVaR Rolling': hCVaR95_rolling_percent.squeeze()})
     hCVaR95_rolling_df.set_index('Date', inplace=True)
     
     #Cálculo de hCVaR al 99% de confianza, creación de dataframe para graficarlo
     hCVaR99_rolling= df_rendimientos[activo_escogido[0]].rolling(window=252).apply(lambda x: x[x <= hVaR99_rolling.loc[x.index[-1]]].mean(), raw=False)
     hCVaR99_rolling_percent = (hCVaR99_rolling * 100).round(4)
     hCVaR99_rolling_df = pd.DataFrame({'Date': df_rendimientos[activo_escogido[0]].index, '99% hCVaR Rolling': hCVaR99_rolling_percent.squeeze()})
     hCVaR99_rolling_df.set_index('Date', inplace=True)
     ...
     ax.plot(VaR99_rolling_df.index, VaR99_rolling_df['99% VaR Rolling'], label='99% Rolling VaR', color='green')
     ax.plot(hVaR95_rolling_df.index, hVaR95_rolling_df['95% hVaR Rolling'], label='95% Rolling hVaR', color='black')
     ax.plot(hVaR99_rolling_df.index, hVaR99_rolling_df['99% hVaR Rolling'], label='99% Rolling hVaR', color='yellow')
     ax.plot(CVaR95_rolling_df.index, CVaR95_rolling_df['95% CVaR Rolling'], label='95% Rolling CVaR', color='pink')
     ax.plot(CVaR99_rolling_df.index, CVaR99_rolling_df['99% CVaR Rolling'], label='99% Rolling CVaR', color='brown')
     ax.plot(hCVaR95_rolling_df.index, hCVaR95_rolling_df['95% hCVaR Rolling'], label='95% Rolling hCVaR', color='blue')
     ax.plot(hCVaR99_rolling_df.index, hCVaR99_rolling_df['99% hCVaR Rolling'], label='99% Rolling hCVaR', color='purple')
